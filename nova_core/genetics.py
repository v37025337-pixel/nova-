...
def genome(bindings, parent=None, engine=None, capabilities=None):
    ...
    if capabilities:
        body.update(schema="nova.genome.v3", capabilities=sorted(set(capabilities)),
                    genes=sorted(set(bindings.values()) | set(capabilities)))
...
def vary(parent, task_id, program, memory):
    ...
    if genome(parent["bindings"], parent["parent"], parent.get("engine"), parent.get("capabilities")) != parent:
    ...
    child = genome({**parent["bindings"], task_id: program["id"]}, parent["id"], parent.get("engine"), parent.get("capabilities"))
